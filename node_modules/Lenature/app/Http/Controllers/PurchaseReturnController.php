...
class PurchaseReturnController extends Controller
{
    ...
    {
        $items = PurchaseReturn::all();
        return view('purchase-return.index')->with(compact('items'));
    }
    ...
    {
        $prefix = 'POR'; // Example prefix
        $dateComponent = now()->format('Ymd'); // Current date component
        $incrementalNumber = PurchaseReturn::count() + 1; // Incremental number

        $invoiceNumber = $prefix . str_pad($incrementalNumber, 4, '0', STR_PAD_LEFT);
        $items = Product::all();
        return view('purchase-return.create')->with(compact('invoiceNumber','items'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $item = PurchaseOrder::where('purchase_invoice_no',$validatedData['invoice_number'])->first();
        ...
        foreach ($validatedData['order_items'] as $itemData) {
            ...
            if ($product) {
                $orderDetail = new PurchaseReturnDetail();
        ...
        $customer = PurchaseOrder::where('purchase_invoice_no', $order->invoice_no)->first();
       
        $order->vendor_id = $customer->vendor_id;
        $order->save();
        $orderData = [
            'customer_name' => $customer->customer->vendor_name,
            'customer_mobile' => $customer->customer->vendor_mobile,
